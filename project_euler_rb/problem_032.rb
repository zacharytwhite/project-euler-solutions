...
def is_pandigital?(arr)
  arr.join.split('').sort.join == '123456789'
end

def find_multiplicands(product, stack)
  1.upto(product**0.5) do |n|
    next unless product % n == 0
    next unless is_pandigital?([n, (product/n), product])
    puts [n, (product/n), product]
    stack << [n, (product/n), product]
  end
end
...
sum = pandigitals.map(&:last).uniq.sum
